package com.lordofduct.util
{
	...
	public class LoDRandom
	{
		...
		/**
		 * A series of random numbers between min and max, each number in the series considers the previous value and forces itself to be pad distance away from it.
		 * 
		 * Passing Number.POSITIVE_INFINITY as numAttempts will guarantee that the list is unique, but the list generation can possibly take a while and you have the threat of an infinite 
		 * loop if the discrete limit of values is less numEntries.
		 */
		public static function randomNextMinMaxSeries(count:int, pad:Number, max:Number, min:Number=0, last:Number=NaN, unique:Boolean=false, numAttempts:int=10):Array
		{
			var arr:Array = new Array();
			var tries:int = 0;
			var value:int = 0;
			
			if (isNaN(last)) last = min;
			
			while (count-- > 0)
			{
				value = randomNextMinMax(last, pad, max, min);
				
				if ( !unique || arr.indexOf( value ) < 0)
				{
					arr.push( value );
					last = value;
					tries = 0;
		...
		public static function randomExcept(funct:Function, params:Array, exceptArray:Array, numAttempts:int = 10):Number
		{
			var value:Number = funct.apply(null, params);
			
			while (exceptArray.indexOf(value) >= 0 && numAttempts-- > 0)
			{
				value = funct.apply(null, params);
			}
			
			return value;
		}
		
		public static function randomList(numEntries:int, funct:Function, params:Array):Array
		{
			var arr:Array = new Array();
			
			while( arr.length < numEntries)
			{
				arr.push(funct.apply(null, params));
			}
