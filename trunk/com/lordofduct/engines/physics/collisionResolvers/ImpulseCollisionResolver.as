package com.lordofduct.engines.physics.collisionResolvers
{
	...
	{
		private static var _inst:ImpulseCollisionResolver;
		
		public static function get instance():ImpulseCollisionResolver
		{
			if (!_inst) _inst = SingletonEnforcer.gi(ImpulseCollisionResolver);
			
			return _inst;
		}
		
		public function ImpulseCollisionResolver()
		{
			SingletonEnforcer.assertSingle(ImpulseCollisionResolver);
		}
/**
 * Class Definition
 */
		...
		public function resolveCollision(result:CollisionResult):void
		{
			...
			var contact:Vector2 = getContacts(body1, body2, normal, penAxis, depth);
			
			resolveImpulse(body1, body2, result.normal, penAxis, contact);
		...
		private function resolveImpulse(body1:IPhysicalAttrib, body2:IPhysicalAttrib, normal:Vector2, penAxis:Vector2, contact:Vector2):void
		{
		...
		private function getContacts(body1:IPhysicalAttrib, body2:IPhysicalAttrib, normal:Vector2, penAxis:Vector2, depth:Number ):Vector2
		{
			...
			axis.copy( cp[ projections[ 1 ] ] );
			axis.lerp( cp[ projections[ 2 ] ], 0.5 );
